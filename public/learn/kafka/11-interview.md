...
1. **멱등성 프로듀서:** `enable.idempotence=true` → PID (Producer ID, 프로듀서 고유 식별자) + SequenceNumber로 중복 감지
...
- Kafka Streams의 State Store (내부적으로 RocksDB - Facebook이 개발한 고성능 임베디드 키-값 저장소)
...
**결론:** Kafka는 이벤트 스트림 저장소로, 실제 DB 대체보다는 DB와 함께 CQRS (Command Query Responsibility Segregation, 명령-조회 책임 분리)/이벤트 소싱 패턴으로 사용합니다.
...
# 2. 인증 (SASL)
# SASL/PLAIN (Simple Authentication and Security Layer - 인증 추상화 계층, 간단, 개발용)
# SASL/SCRAM (권장)
...
# 3. 권한 관리 (ACL)
# ACL (Access Control List, 접근 제어 목록) 설정
