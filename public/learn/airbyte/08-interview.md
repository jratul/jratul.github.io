...
- BigQuery, Snowflake 같은 클라우드 DW는 대규모 SQL 변환이 매우 빠름
- 원본 데이터를 Raw로 보존해 재처리 가능
- 변환 로직을 DW 내에서 dbt로 관리 (버전 관리, 테스트 용이)
...
CDC(Change Data Capture)는 데이터베이스의 **트랜잭션 로그(Binlog/WAL)**를 읽어 INSERT/UPDATE/DELETE를 모두 실시간으로 캡처하는 방식입니다.
...
- CDC 모드: Binlog Position 또는 LSN — 다음 sync 시 이 위치부터 읽음
...
| 커넥터 수 | 300+ | 300+ |
| SLA 보장 | Community 지원 | 엔터프라이즈 SLA |
