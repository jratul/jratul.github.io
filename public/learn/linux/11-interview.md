...
- IPC(Inter-Process Communication, 프로세스 간 통신)로 통신
- 생성/전환 비용이 큼 (`fork()`)
...
# 리다이렉션
command > file.txt     # stdout(표준 출력) → 파일 (덮어쓰기)
command >> file.txt    # stdout → 파일 (이어쓰기)
command 2> error.log   # stderr(표준 에러) → 파일
...
# DNS(Domain Name System, 도메인 이름 시스템) 조회
...
iostat              # I/O(Input/Output, 입출력) 통계
sar                 # 시스템 활동 기록
```
