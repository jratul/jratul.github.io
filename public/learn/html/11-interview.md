...
**중요한 이유:**
- **접근성:** 스크린 리더가 구조를 이해해 시각 장애인에게 더 나은 탐색 제공
- **SEO:** 검색 엔진이 콘텐츠 중요도를 파악
...
DOCTYPE이 없으면 브라우저는 **Quirks Mode**로 렌더링합니다:
- 오래된 IE 호환 방식으로 렌더링
- CSS 박스 모델 계산 등이 표준과 다르게 동작

HTML5의 DOCTYPE은 매우 단순합니다 — 이전 HTML 4.01이나 XHTML처럼 DTD 참조가 필요 없습니다.
