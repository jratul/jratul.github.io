...
## Q1. EC2(Elastic Compute Cloud, 클라우드 가상 서버 서비스)의 인스턴스 타입과 구매 옵션을 설명해주세요
...
| i (Storage) | 고속 NVMe SSD(고속 스토리지 인터페이스) | DB, 데이터 웨어하우스 |
...
## Q2. VPC(Virtual Private Cloud, 가상 사설 클라우드 네트워크)의 주요 컴포넌트를 설명해주세요
...
- **NACL(Network Access Control List, 네트워크 접근 제어 목록):** 서브넷 레벨, Stateless (인/아웃 각각 설정)
...
## Q3. S3(Simple Storage Service, 객체 스토리지 서비스)의 스토리지 클래스를 설명해주세요

| 클래스 | 접근 빈도 | 가용성 | 비용 |
|-------|---------|-------|-----|
| Standard | 자주 | 99.99% | 높음 |
| Standard-IA | 가끔 | 99.9% | 중간 |
| One Zone-IA | 가끔 (단일 AZ(Availability Zone, 가용 영역)) | 99.5% | 낮음 |
...
**S3 보안:**
```
버킷 정책 + IAM(Identity and Access Management, 접근 권한 관리 서비스) 정책 + ACL(레거시)
...
## Q4. RDS(Relational Database Service, 관계형 데이터베이스 관리 서비스)와 Aurora의 차이는?

| 비교 | RDS | Aurora |
|-----|-----|--------|
| 호환 | MySQL, PostgreSQL, Oracle, SQL Server | MySQL, PostgreSQL 호환 |
| 성능 | 표준 | MySQL의 5배, PostgreSQL의 3배 |
| 스토리지 | 단일 EBS(Elastic Block Store, 블록 스토리지), 수동 증가 | 자동 증가 (최대 128TB) |
| 복제 | 싱글 AZ / Multi-AZ | 3 AZ × 2 복사 = 6개 복사본 자동 |
| 장애 복구 | ~1-2분 | ~30초 이하 |
| 비용 | 낮음 | RDS 대비 높음 |

**Aurora Serverless v2:** 용량을 자동으로 조절 (ACU(Aurora Capacity Unit, Aurora 용량 단위) 단위), 간헐적 워크로드에 적합.
...
Pub/Sub(발행/구독 메시징 패턴): 실시간 알림, 채팅
분산 락: SETNX 또는 Redlock 패턴
```

**Read-Through 캐시 패턴 (캐시에 없으면 DB를 조회하고 결과를 캐시에 저장하는 방식):**
```
1. 캐시 조회 → 캐시 미스
2. DB 조회
3. 결과를 캐시에 저장 (TTL(Time To Live, 캐시 유효 시간) 설정)
4. 응답 반환
```
...
**해결 방법:**
- **Provisioned Concurrency(사전 할당 동시성):** 미리 컨테이너 워밍업 (비용 발생)
...
## Q7. ECS(Elastic Container Service, 컨테이너 오케스트레이션 서비스)와 EKS(Elastic Kubernetes Service, 관리형 쿠버네티스 서비스)의 차이를 설명해주세요
...
```
1. 루트 계정 사용 금지 — MFA(Multi-Factor Authentication, 다중 인증) 활성화 후 잠금
...
**Permission Boundary(권한 경계):** IAM 엔티티가 가질 수 있는 최대 권한 한도 설정 (위임 시 권한 상승 방지)
