...
Kubernetes(K8s) 운영/개발 면접에서 빈출되는 핵심 질문들입니다.
...
⚠️ Secret은 기본적으로 base64 인코딩만 됩니다 (암호화 아님). 프로덕션에서는 RBAC + 암호화(etcd), 또는 Vault/External Secrets 사용을 권장합니다.

---

## Q5. HPA(Horizontal Pod Autoscaler)는 어떻게 동작하나요?
...
| Probe | 실패 시 동작 | 목적 |
|-------|-----------|------|
| **Liveness** | 컨테이너 재시작 | 데드락, 메모리 누수 감지 |
...
**Ingress:** L7 로드밸런서, 도메인/경로 기반 라우팅
...
## Q8. RBAC(Role-Based Access Control)을 설명해주세요
