...
Heap은 다시 Young(Eden + Survivor) / Old Generation으로 나뉘며, GC 전략이 영역별로 다릅니다.
...
- Old 영역이 가득 찼을 때 발생
- STW(Stop-The-World) 시간이 길어 성능에 영향
...
| Parallel GC | 멀티 스레드 Minor GC | throughput 중시 (Java 8 기본) |
| G1 GC | 힙을 Region으로 분할, 예측 가능한 pause | Java 9+ 기본 |
...
| `finalize()` | GC 직전 호출되는 Object 메서드 (Java 9부터 deprecated) |
...
**제네릭 장점:**
- 컴파일 타임 타입 안전성 — ClassCastException 방지
- 캐스팅 코드 제거
- 재사용성 증가
...
Spring/JPA에서는 대부분 RuntimeException을 사용해 unchecked로 처리합니다.
...
- 상호 배제(mutual exclusion) + 가시성(visibility) 보장
...
**AtomicInteger**는 CAS(Compare-And-Swap)로 락 없이 원자성을 보장합니다.
...
✅ 선언적이고 읽기 쉬움
✅ 지연 평가(lazy evaluation) — 필요할 때만 연산
